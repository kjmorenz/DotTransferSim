...
def nextphotonss(lastphoton, sensitivity,
    ...
    # and so there might be too many "if"s slowing down the code
    # 
    ...
    for i in range(numEms):
        ...
        while nextem < endround: #continue adding photons until end of round, on average 10 emissions
            ...
            while semtime > fisstime and transtime > temtime:
                
                if pulsed == 1:
                    nextdex = int(nextem/taurep)*taurep + numpy.random.geometric(p=probdex)*taurep 
                    nextlex = int(nextem/taurep)*taurep + numpy.random.geometric(p=problex)*taurep
                    ...
                    nextlex = nextem + scipy.random.exponential(k_lexcitation)
                ...
                semtime = nextlex + scipy.random.exponential(k_sem)
                fisstime = nextlex + scipy.random.exponential(k_fiss)
                transtime = fisstime + scipy.random.exponential(k_trans)
                temtime = fisstime + scipy.random.exponential(k_tem)
